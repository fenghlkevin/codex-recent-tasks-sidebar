...
- 增强 Codex 智能日报与周报：日报保留逐项成果和完成度，周报按项目合并为数句概述，并保留明确出现的 Bug 编号。
- 新增简洁、标准、管理三种报告风格，支持编辑结果、保存本地历史、复制 Markdown 和导出报告。
- 新增报告项目业务别名与排除规则；设置使用路径哈希保存，排除项目不会进入报告上下文。
- 报告窗口打开或切换日报/周报时不会自动生成，只有明确点击“生成报告”后才读取所选时间范围内的脱敏上下文。
- 生成过程显示当前阶段、估算进度和已用时间，Codex 不可用时回退到本地摘要。
- 修复报告工具栏窄空间文字折行，并完善报告历史脱敏和固定数据自检。
...
- 自动按今天或本周筛选任务，并按项目分组。
- 以用户提出的需求为主、助手处理结果为证据：日报归纳每条工作的一句话成果和完成度；周报按项目概括 Bug 修复、问题处理、新增功能和其他明确推进，并保留上下文中明确出现的 Bug 编号。
- 自动生成工作总结及下一工作日/下周建议。
- 生成前可选择简洁、标准或管理三种报告风格；切换风格不会自动开始生成。
- 生成后可直接编辑逐项成果、项目概述、工作总结和后续建议。
- 可把最终报告保存到本地历史，重新打开或删除；最多保留 50 份。
- 生成过程中显示当前阶段、估算百分比和已用时间；模型返回完整结果前不会显示 100%。
- 支持复制 Markdown 或导出 `.md` 文件。

任务右键菜单还可以设置报告使用的项目业务名称，或将项目从日报、周报中排除。别名只影响报告，侧边栏继续显示原项目名；排除规则会在读取该项目的报告上下文之前生效。
...
- ChatGPT 进入 macOS 全屏 Space 时会隐藏侧边栏，退出全屏后按原吸附或置顶模式恢复。
- 在单显示器上左右切换 macOS Space 时，侧边栏会在过渡期间隐藏，避免闪到相邻桌面。
- ChatGPT 窗口跨显示器移动时，侧边栏会先隐藏，待窗口在目标屏幕稳定后一次性重新吸附。
...
- 可展开查看近 30 天 token 趋势、等效 API 成本估算和限额重置额度。
...
- 项目报告设置以项目路径哈希为键，不在设置中保存绝对路径。报告历史只保存最终报告，写入前会移除 Thread ID，并把项目路径替换为哈希；不保存原始对话和工具日志。
- 不打印或保存登录令牌和原始账号响应。
- 日志只记录数据库候选位置、文件状态、SQLite 错误码和重试结果，不记录任务内容。
- 本地设置仅使用 `UserDefaults` 保存窗口位置、宽度、吸附侧、显示模式、用量面板状态、报告风格，以及以路径哈希为键的项目报告设置。
